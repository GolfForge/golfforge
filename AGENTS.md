...
1. **Re-import `layer_cart_path.png` on Windows (UE-side).** The fixed PNG is in `courses/bethpage-black/layer_cart_path.png` after `git pull`. Open the Bethpage landscape, Landscape > Manage > Import, uncheck Heightmap, set CartPath row to the new PNG, Import. Visual check: cart paths should now appear as thin grey strokes following actual paths, not large filled regions.
   - **Decision pending** between (a) keeping the raster path (works today, simple, slightly aliased at landscape resolution) vs (b) switching to **`LandscapeSplineActor`** consuming `cart_path.geojson` (40 LineString features now emitted alongside the PNG). Splines preserve sharp edges + width, look much better at close-up zoom, and let UE deform the terrain along the path. Recommend trying the raster import first; if it still reads poorly even at correct width, switch to splines.
