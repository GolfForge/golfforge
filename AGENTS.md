...
1. **Manual physics drop verification.** With `BethPageBlack` open, click Simulate (or Alt+S) in the editor viewport. `DropSphere` should fall ~30m, hit the green terrain, roll. Confirms the landscape collision is wired and Z scale is sane.
2. **Milestone 0.5.** Add splatmap-driven material layers using `courses/bethpage-black/splatmap.png` (RGBA = fairway/green/bunker/rough). Will need to convert `M_GolfsimGreen` from a single-color material to a layer-blend material with weight-painted samples. The bridge's `import_texture` handles the PNG; the splatmap → LandscapeLayerInfo → paint flow will need its own playbook (likely manual in the Landscape paint mode for the first pass).
3. **Ball flight aerodynamics solver (C++).** Drag + gravity + Magnus lift. Tune against Trackman reference numbers. Probably a week of careful work.
