...
1. **Milestone 0.7 (extras): cart paths, tees, trees, water.** Pipeline already produces `layer_cart_path.png`, `layer_tee.png`, `layer_trees.png`, `layer_water.png` as single-channel PNGs. Cart paths and tees can land on a second material slot or as a per-layer addition; trees become PCG-scattered foliage instances; water is a separate water-body actor. Different patterns per layer — won't ship as one Python script.
2. **Material polish.** M_GolfsimCourse uses flat colors (M0.5 milestone was layer plumbing, not visuals). Upgrade to tiled textures + normal maps per layer for course realism. Keep the layer-blend topology, swap the `Constant3Vector` inputs for `TextureSample` chains.
