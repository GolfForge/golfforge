...
1. **Pipeline: dilate cart-path line geometry before rasterization.** OSM cart paths come as polylines; rasterizing them at the heightmap's ~70 cm/pixel makes them ~1 pixel wide, which barely registers in the painted landscape. In `build_splatmap.py`, buffer the cart-path geometry by ~2-3 m (a few pixels) before drawing into `layer_cart_path.png`. Lives on Mac per [[feedback-pipeline-runs-on-mac]]. Re-run for Bethpage Black after.
