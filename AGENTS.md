...
1. **Switch to Claude Code for engine work.** Install (`npm install -g @anthropic-ai/claude-code`), copy `.mcp.json.example` → `.mcp.json` at repo root, run `claude` from the repo. In the REPL, `/mcp` confirms the `unreal-claude-mcp` server is connected. Cowork sandbox does NOT see custom MCPs; Claude Code does. See `docs/windows-setup.md` "Pick a Claude surface" section.
2. **MCP smoke test.** With UE editor running (TCP server on 127.0.0.1:18888) and Claude Code launched: *"Spawn a cube actor at (0, 0, 200) in my UE editor and label it SmokeCube."* Cube appearing in the World Outliner = bring-up gate passed.
3. **Milestone 0.** Import `courses/bethpage-black/heightmap.png` into UE5 as a Landscape (Z scale 8.42% from `heightmap.json`). Apply a single default material (no splatmap-driven layers yet). Spawn a physics sphere. Watch it roll. That's the milestone. With the MCP wired in Claude Code, this is mostly: ask Claude to use `import_texture` + `execute_unreal_python` to create the Landscape from the PNG, then `spawn_actor` for the sphere.
4. **Milestone 0.5.** Add splatmap-driven material layers using `courses/bethpage-black/splatmap.png` (RGBA = fairway/green/bunker/rough). Verify the course visibly separates into surface types.
