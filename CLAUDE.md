...
- **Latest:** **GOL-205 OldAndre links vibe pass + full playable course DONE (2026-06-11, Windows).** OldAndre went from putting-only to a **full 18-hole playable links**. Cleared inherited demo dressing (10 stale demo `Water_*`), ran the vibe pass: 5 real OSM water hazards, ambient (`place_ambient_sfx.py` now recognizes OldAndre), bright-afternoon `AGolfEnvironment`, and a **branched links material** `MIC_GolfsimLinks` (2nd MI of the shared base via new `build_oldandre_material.py`; demo untouched). **Rebuilt the map** at a loop-inclusive ~2.2 km bbox (`-2.8318,56.342,-2.7962,56.3618`, cog NO41NE+NO51NW) — the GOL-199 bbox clipped the Old Course's far loop (holes 7-9,11 were up to 136 m off the north edge); re-imported heightmap (Z 5.76) + splats, re-applied the vibe pass (13.1k world-up trees). All **18 holes on-terrain, par 72**, incl. the real hole 8. Made it playable: `oldandre` card in `GolfCourseRegistry`; the 89-hole whole-complex `hole.geojson` filtered to the Old's 18 by `course:name="Old"` (`ParseHoleScheduleJson` reads `course:name` fallback; `DeriveTrackName("oldandre")->"Old"`). Round tests 19/19. **Deferred to a 2nd pass:** on-green approaches break ~3 m on steep (6°) green spots — diagnosed as real slope-driven fall-line break (not a teleport); fix = clamp break slope + kernel revisit (GOL-195-adjacent).
- **Active focus — alpha-2 gameplay foundation.** The alpha-3 visual vibe pass is done; now make it *play* like a course. **(1) Bunkers — GOL-34 DONE. (2) Ball physics — GOL-93:** GOL-38 multi-bounce + GOL-39 cross-surface/spin-back + GOL-196 terrain-aware bounce DONE; remaining is **GOL-195** (per-surface coefficient calibration vs Square Omni LM data + promote `FSurfaceRoll` to a tunable UDataAsset/UDeveloperSettings) — **LM-gated, deferred**. **(3) Practice mode — GOL-73 closest-to-pin DONE** (carry + flat-green putt-out). **(4) Pin positions — GOL-191/192 DONE** (drop the course disc + Static/Random/Tournament pins from `green.geojson`; gimme-ring polish). **(5) Putting — GOL-75 + GOL-199 DONE:** physics + practice-mode logic + **putt-on-a-real-green on the OldAndre links (St Andrews Old Course, whole ~2 km map)** all shipped — cup capture, ball-at-address, lie-correct re-putts, banner, real break on the 18th. **GOL-204 trees-to-world-up DONE** + **GOL-205 OldAndre full playable links DONE.** **NEXT (prioritized): green-roll 2nd pass** (the deferred on-green over-break: clamp the fall-line slope + revisit the normal-smoothing kernel; LM-gated calibration is GOL-195), then **GOL-203** (elevate putting: UMG hole-out toast+SFX, putt camera, ground tracer, capture tuning), **GOL-202** (practice-UI gating). Also: **GOL-201** famous-greens tour (OldAndre has 84 greens to hop), **GOL-200** break-aware line preview. Next practice ticket: islands **GOL-74**.
