...
- **Latest:** **GOL-151 — Menu/HUD icon set: Lucide icon font** (2026-06-03, Windows). Replaced every placeholder UI glyph (`→`/`✓`/`↵`-as-"Enter"/`±`/empty boxes) with real **Lucide** line icons rendered from a font (UMG has no SVG renderer). Shipped `lucide-static@0.544.0`'s **prebuilt** `lucide.ttf` (716KB, ISC + Feather-MIT, license bundled as `LICENSE-Lucide.txt`) — no subset/generation (`fantasticon` is broken on Windows; recorded in cookbook). Imported via MCP Python (GOL-138 recipe): `UFontFace` `Faces/Lucide-Regular` (LazyLoad) + runtime `UFont` `/Game/UI/Fonts/Lucide` wired via `CompositeFont.import_text`. `GolfUITheme` gained `Icon(size)`, `enum class EIcon` (15 glyphs at Lucide's PUA codepoints), `MakeIcon(...)`, `MakeKbd(EIcon)`. Full sweep: menu (arrow/↵/power/brand-flag/clock-cloud), leave dialog (log-out/arrow/↵), course+option checks, HUD conditions (wind/temp/sky/time), wizard ∓. `/Game/UI` already always-cooked. Full-build + 63/63 tests green. History → `docs/worklog.md`.
- **Active focus:** (1) **v0.0.4-alpha release** — combined **Win + Mac cook** now that the whole UI epic + icons have landed, then tag + GitHub Release; gated on **GOL-49** (`engine/scripts/package.sh` = BuildCookRun + post-cook `courses/` copy) and a cook-time check that the new `/Game/UI` fonts (incl. **Lucide**)/materials stage. (2) **Remaining UI polish/follow-ups:** **GOL-152** (rename Golfsim→GolfForge, cross-machine), **GOL-153** (stepper pill repaint), **GOL-154** (live weather → conditions wind/temp), plus **dev/secondary panel reskins** (cheat sheet, shot history, previous-sessions, manual-shot dialog) and a **reduced-motion a11y setting**.
