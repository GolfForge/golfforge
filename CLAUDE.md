...
- **Latest:** **GOL-168 range yardage-marker stakes (first pass)** (2026-06-05, Windows). New `engine/scripts/place_range_props.py` spawns painted-stake distance markers (white post + red cap + yellow `TextRenderActor` number) at 50/100/150/200/250 yd on both fairway edges of the Practice Range. Reuses the `AGolfRangeHUD::ApplyPinDistance` convention (`X = TeeX + yd*91.44`, TeeX from the range PlayerStart; off-centerline `Y = ±2286` cm so stakes stay clear of ball flight + the target pin); ground-snapped, idempotent (kill-by-label), `RangeProps` layer, numbers `yaw 180` to face the tee. Verified 30 actors placed, no dupes on re-run. **Persistent** actors — operator must Save `PracticeRange.umap`. Procedural/code-only (no Fab). **GOL-168 stays open:** Fab backdrop (GOL-30), tee-box benches/ball washers, per-hole course props (yardage markers, hole signs, clubhouse) all deferred pending Fab meshes.
- **Active focus (alpha-3, GOL-160 vibe pass):** flag (GOL-165) + materials (GOL-163) done; props (GOL-168) range first-pass done, remainder deferred. Remaining children: **GOL-164** water polish, **GOL-166** ambient SFX, **GOL-167** tree species, **GOL-168** remainder (Fab-dependent: backdrop/benches/course props); optional GOL-163 **P4** per-region noise. **Deferred:** **GOL-173** tree sway (later epic); **GOL-171** course layer-data audit (mac) + **GOL-172** pipeline sanity-overlay (mac) + any engine fix for the splat→landscape mapping; **GOL-170** 3D grass; **GOL-169** night; **GOL-27** range R4. Siblings: **GOL-93** ball physics, **GOL-94** course authoring. Other: GOL-152 rename, GOL-153/154, GOL-158/159 (beta).
