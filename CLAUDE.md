...
- **Active focus:** (1) **GOL-157 — v0.0.4-alpha macOS cook (Mac mini).** Cook `GolfForge-macos-arm64.zip` and `gh release upload` it to the already-published `v0.0.4-alpha` release. The ticket has the full recipe; the two gotchas are the **GOL-71 dylib fixup** (copy `libtbb*.dylib` into `GolfForge.app/Contents/MacOS/` or the `.app` crashes on launch) and the **first-ever Mac-with-course `courses/` staging path** (resolve where `CoursePaths` lands inside a `.app`). (2) **Remaining UI polish/follow-ups:** **GOL-152** (rename Golfsim→GolfForge, cross-machine — incl. the real exe/module rename), **GOL-153** (stepper pill repaint), **GOL-154** (live weather → conditions wind/temp), plus **dev/secondary panel reskins** (cheat sheet, shot history, previous-sessions, manual-shot dialog) and a **reduced-motion a11y setting**.
