...
- **Latest:** **GOL-206 green-roll 2nd pass DONE (2026-06-11, Windows).** Fixed the "ball settles then runs 1-2 m to a random spot" on greens (GOL-205's deferred issue — real fall-line break on 6° LIDAR bank cells classified green, not a teleport). New per-surface `FSurfaceRoll::BreakSlopeMaxDeg` (default 45° = uncapped) clamps the roll fall-line slope term in `GroundRoll.cpp`; **Green = 3.5°** (a 6° cell now breaks 0.106 m, was ~3 m; sub-clamp slope still breaks progressively). **Putts deliberately keep full break** (`PutterSurfaceRoll` stays 45° — authentic steep-feature swing; the field rides the GOL-195 data-asset promotion). Also a **low-speed settle floor** (`RollSettleSpeedMps = 0.05` replaces the 1e-6 roll-exit; kills slope-fed creep, sub-mm rest shift) and the normal kernel stays 0.75 m/5-point (why documented in `CourseSurfaceSubsystem.cpp`). 3 new GroundRoll tests; full Golfsim suite green.
- **Active focus — alpha-2 gameplay foundation.** The alpha-3 visual vibe pass is done; now make it *play* like a course. **(1) Bunkers — GOL-34 DONE. (2) Ball physics — GOL-93:** GOL-38 multi-bounce + GOL-39 cross-surface/spin-back + GOL-196 terrain-aware bounce DONE; remaining is **GOL-195** (per-surface coefficient calibration vs Square Omni LM data + promote `FSurfaceRoll` to a tunable UDataAsset/UDeveloperSettings) — **LM-gated, deferred**. **(3) Practice mode — GOL-73 closest-to-pin DONE** (carry + flat-green putt-out). **(4) Pin positions — GOL-191/192 DONE** (drop the course disc + Static/Random/Tournament pins from `green.geojson`; gimme-ring polish). **(5) Putting — GOL-75 + GOL-199 DONE:** physics + practice-mode logic + **putt-on-a-real-green on the OldAndre links (St Andrews Old Course, whole ~2 km map)** all shipped — cup capture, ball-at-address, lie-correct re-putts, banner, real break on the 18th. **GOL-204 trees-to-world-up DONE** + **GOL-205 OldAndre full playable links DONE** + **GOL-206 green-roll 2nd pass DONE.** **NEXT (prioritized): GOL-203** (elevate putting: UMG hole-out toast+SFX, putt camera, ground tracer, capture tuning), then **GOL-202** (practice-UI gating). Also: **GOL-201** famous-greens tour (OldAndre has 84 greens to hop), **GOL-200** break-aware line preview. Next practice ticket: islands **GOL-74**.
