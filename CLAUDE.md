...
- **Active focus:** **EventBus (GOL-7) + manual-shot dialog (GOL-8) shipped** — the bus and its first producer are in, so the real launch-monitor + sim tickets are now tractable. Tracks from here:
  - **Launch-monitor track (High, Windows):** **GOL-11 OpenFlight** — the first *real* LM driver: connect to OpenFlight's WebSocket, normalize its shot JSON into `FShotTakenEvent`, publish on the bus (AGPL-isolated). The dev-only localhost Python driver (`ws://localhost:7878/events`) falls out of the same network-ingestion + JSON↔struct serialization, so it's a small add-on afterward. Natural next pick on this machine.
