...
- **Latest:** **GOL-204 PCG trees orient to world-up DONE (2026-06-11, Windows).** Trees leaned with the terrain on slopes because the landscape surface sample hands each point a normal-aligned rotation and the scatter graph added our yaw+lean *relative* to it. One-line fix in `build_pcg_treescatter.py::_mk_transform` — `absolute_rotation=True` on the TransformPoints node — so the rolled rotation (world-up + small random pitch/roll + random yaw) **replaces** the normal. The single `_mk_transform` feeds both species branches and both graphs (course `/Game/PCG/PCG_TreeScatter` + range `/Game/PCG/PCG_TreeScatter_Range` via `build_range_treescatter.py`). `GenerateOnLoad` does NOT re-bake on load (instances serialize per-umap), so all **7** tree-bearing umaps were force-regenerated + re-saved (GolfForgeDemo Black/Blue/Green/Red/Yellow, OldAndre, PracticeRange). Verified world-up numerically (trunk up-vector from the per-instance quaternion): max tilt 3.93–4.12° per map (inside the ±3°/±2° lean envelope), was up to ~42° on slopes; counts unchanged. Touch points: 1 script + 2 graph `.uasset`s + 7 `.umap`s.
- **Active focus — alpha-2 gameplay foundation.** The alpha-3 visual vibe pass is done; now make it *play* like a course. **(1) Bunkers — GOL-34 DONE. (2) Ball physics — GOL-93:** GOL-38 multi-bounce + GOL-39 cross-surface/spin-back + GOL-196 terrain-aware bounce DONE; remaining is **GOL-195** (per-surface coefficient calibration vs Square Omni LM data + promote `FSurfaceRoll` to a tunable UDataAsset/UDeveloperSettings) — **LM-gated, deferred**. **(3) Practice mode — GOL-73 closest-to-pin DONE** (carry + flat-green putt-out). **(4) Pin positions — GOL-191/192 DONE** (drop the course disc + Static/Random/Tournament pins from `green.geojson`; gimme-ring polish). **(5) Putting — GOL-75 + GOL-199 DONE:** physics + practice-mode logic + **putt-on-a-real-green on the OldAndre links (St Andrews Old Course, whole ~2 km map)** all shipped — cup capture, ball-at-address, lie-correct re-putts, banner, real break on the 18th. **GOL-204 trees-to-world-up DONE** (all maps re-scattered). **NEXT (prioritized) = GOL-205** (clear OldAndre's inherited demo trees/props + links vibe pass), then **GOL-203** (elevate the putting experience: UMG hole-out toast+SFX, putt camera, ground tracer, capture tuning), **GOL-202** (practice-UI gating). Also: **GOL-201** famous-greens tour (the OldAndre data already has 84 greens to hop), **GOL-200** break-aware line preview. Next practice ticket: islands **GOL-74**.
