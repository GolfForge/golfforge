...
- **Latest:** **GOL-191/192 pin-position system DONE (2026-06-10, Windows).** Course pins drop the synthetic green disc+collar (`AGolfPinActor::SetGreenSurfaceVisible(false)`) — the painted `splat_green` landscape is the green. New `EPinMode {Static, Random, Tournament}` on `FRoundConfig` + a Pin-positions picker in the round-setup Format step; pure `GolfsimRound::` resolver (parse `green.geojson` polygons + pin sheets, point-in-polygon, random-in-green, green↔hole match, `ResolvePinPositions`) baked into the schedule at `URoundSubsystem::StartRound`. **Random** = a fresh on-green spot each round; **Tournament** = `courses/<id>/pins/<id>.json` (real event hole locations; `golfforge-demo-black` is Bethpage Black) with centroid/static fallbacks. `pipeline/build_pin_sheet.py` seeds a baseline sheet (sample `pins/default.json`, 18 pins). **Gimme ring upgraded** to a terrain-conforming **dashed translucent halo** (procedural annulus, 8 dashes, `M_GimmeRing` 38% opacity) with a flat **"GIMME"** label in the gap. 5 new tests; full suite **88/88**. Follow-ups: real pin-sheet content, multi-sheet dropdown, Static-pin centroid.
- **Active focus — alpha-2 gameplay foundation.** The alpha-3 visual vibe pass is done; now make it *play* like a course. **(1) Bunkers — GOL-34 DONE. (2) Ball physics — GOL-93:** GOL-38 multi-bounce + GOL-39 cross-surface/spin-back DONE; remaining is **GOL-195** (per-surface coefficient calibration vs Square Omni LM data + promote `FSurfaceRoll` to a tunable UDataAsset/UDeveloperSettings) — **LM-gated, deferred**. **(3) Practice mode — GOL-73 closest-to-pin DONE** (carry + flat-green putt-out). **(4) Pin positions — GOL-191/192 DONE** (drop the course disc + Static/Random/Tournament pins from `green.geojson`; gimme-ring polish). Next practice tickets: islands **GOL-74**, putting **GOL-75** (real green *surface/break* still pending — the painted green + polygon are in, but no sampled break model). **Parallel — UI cleanup:** tracer UI (GOL-107), reskin secondary panels onto `GolfUITheme` (GOL-155), round-setup pills (GOL-153).
