...
- **Latest:** **GOL-145 — In-round HUD: telemetry readout + control bar + §6 LM gating** (2026-06-03, Windows). Ninth child of the GOL-137 UI epic; partner to GOL-144 (top panels). Rebuilt `UGolfRangePanel` from a top-right grid into the spec's **bottom** layout (`06`/`08`): a **telemetry readout** (bottom-left — club + ball/launch/spin + carry-accent/total/offline, count-up intact) + a **full-width control bar** (Club/Time/Sky/Camera/Launch-Monitor dropdowns + an LM status pill), shown on **both** range + round (RoundHud layers on top in-round; pin/putt are a range-only cluster). Wired **§6 gating**: new `ELaunchMonitorStatus {Sim,Off,Pairing,Online}` on the manager/driver layer drives `EInputMode` (Online → Simulation: device owns the stream, "Sim shot", meter hidden, green pill; else → Game: keyboard swing, "Swing", meter shown, amber pill), resets the swing on every LM change; default = "Simulated (no device)". Device list stays real-only — the status enum is the seam the **incoming Square Omni / Blue Tees Rainmaker** drivers plug into. Commit `19cdb3d` (full-build + 63/63 tests + PIE verified). **GOL-137: 9/12 children done (138/139/150/140/141/142/143/144/145) — history in `docs/worklog.md`.**
- **Active focus:** (1) **GOL-137 UI elevation** (High) — next: **GOL-146** (swing meter glass reskin — **also reposition it clear of the new GOL-145 control bar**, which it currently overlaps), then **GOL-147** (leave/quit modal — wires the round panel's Menu guard), **GOL-148** (scorecard reskin + epic closeout). Also queued: **GOL-151** (Lucide icon font — retires the placeholder control-bar/condition/avatar icons), **GOL-152** (rename Golfsim→GolfForge, cross-machine), **GOL-153** (stepper pill repaint), **GOL-154** (live weather → conditions wind/temp). (2) **v0.0.3-alpha release** — Mac cook (`machine/mac`) then tag + GitHub Release. (3) **GOL-49** — `engine/scripts/package.sh` bundling BuildCookRun + the post-cook `courses/` copy.
