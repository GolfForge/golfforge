...
- **Latest:** **GOL-196 terrain-aware bounce DONE (2026-06-10, Windows).** Balls bounce off the slope, not flat ground: `GroundRoll.cpp::SimulateGroundRollCrossSurface` reflects each touchdown's velocity about the surface **normal** (normal comp × Restitution, tangential × BounceHorizontalKeep), tracking a running 2D position since the heading deflects per hop. Flat normal (0,0,1) reproduces the old straight bounce exactly (equivalence test guards it). New `UEventBusSubsystem::GroundNormalProvider` (mirrors `SurfaceProvider`); range HUD + course subsystem trace the landscape `ImpactNormal` (range unrotates by aim → launch-local; course local==world). Down-slope kicks forward + runs long, up-slope pops + short, side-slope throws lateral. New `Golfsim.GroundRoll.SlopeBounceDeflection` test; full suite **89/89**. Coefficients stay code-seeded (GOL-195); roll-follows-fall-line deferred (GOL-75).
