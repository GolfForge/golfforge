...
- **Latest:** **GOL-39 cross-surface roll + green spin-back DONE (2026-06-09, Windows).** New pure-SI `GolfBallFlight::SimulateGroundRollCrossSurface(Flight, SurfaceAt, CoefsFor)` (`Physics/GroundRoll.cpp`) re-samples the surface via callbacks at each bounce/roll step and swaps `FSurfaceRoll` coefficients at boundaries — a ball rolling fairway→bunker now drags to a stop in the sand (the gameplay half of GOL-34). The old closed-form roll became a stepped integrator that **telescopes to `Vh²/(2a)` exactly**, so single-surface `SimulateGroundRoll` (now a thin wrapper) is numerically identical (driver still 279.7 yd). **Green spin-back**: high backspin + steep descent on a green checks then rolls *backward* (new `SpinBackGain` knob; test: 9000 rpm ends 4.06 m behind the pitch mark). `EventBusSubsystem` full shots use the cross-surface path; putts stay single-surface. 3 new tests; `Golfsim.GroundRoll` 12/12 + full suite green. Coefficients stay code-seeded; calibration + tunable layer deferred → GOL-195.
- **Active focus — alpha-2 gameplay foundation.** The alpha-3 visual vibe pass is done; now make it *play* like a course. **(1) Bunkers — GOL-34 DONE. (2) Ball physics — GOL-93:** GOL-38 multi-bounce + GOL-39 cross-surface/spin-back DONE; remaining is **GOL-195** (per-surface coefficient calibration vs Square Omni LM data + promote `FSurfaceRoll` to a tunable UDataAsset/UDeveloperSettings) — **LM-gated, deferred**. **(3) Practice mode — GOL-73 closest-to-pin (NEXT)** now physics has landed (islands GOL-74 / putting GOL-75 deferred — putting waits on green geometry GOL-191). **Parallel — UI cleanup:** tracer UI (GOL-107), reskin secondary panels onto `GolfUITheme` (GOL-155), round-setup pills (GOL-153).
