...
- **Latest:** **GSPro connector behavior profiles + Springbok** (2026-06-06, Windows). The GSPro Open Connect driver (GOL-178) shipped + is live-validated against **squaregolf** and **springbok** (2 of 6, GOL-181). Bringing up springbok proved the open-source connectors don't implement the protocol identically, so the driver was refactored to **per-connector behavior profiles** — `FGSProConnectProfile` (`bArmModel`, `RearmSettleSeconds`, `bSendPlayerOnConnect`, `DefaultClub`) set per dropdown entry (`gsproconnect`/`squaregolf`/`springbok`), so tuning one connector can't break another. Universal fixes: a **streaming brace-depth JSON extractor** (`ExtractJsonObjects`) handles both newline-delimited (squaregolf) and concatenated (springbok) framing; `{Code:201}` always carries `Player.Club` (springbok `KeyError`s otherwise); re-arm gated to `bArmModel`; `Backspin`/`BackSpin` casing tolerated. Clean UE 5.7 build; **10** `Golfsim.GSProConnect.*` tests; springbok fired realistic shots (159 mph/15.8°/2788 rpm → 235 m) live. Adding a connector = one profile. Follow-ups: ball-ready UI **GOL-186**, club/face-impact **GOL-187**; full 6-connector matrix **GOL-181**.
- **Active focus:** **LM connectors (GOL-97 epic):** GOL-178 driver done → **GOL-181** validate the 6 community connectors against the listener (gate before public claim); **GOL-180** native-connector epic for auth-gated devices (R50/Launch Pro/GCQuad). **alpha-3 GOL-160 vibe pass:** flag (GOL-165) + materials (GOL-163) + atmosphere (GOL-161) + water (GOL-164) + tree species (GOL-167) + ambient SFX (GOL-166) done; props (GOL-168) range first-pass done. Remaining: **GOL-168** remainder + **GOL-175** water caustics; optional GOL-163 **P4** per-region noise. **Deferred:** **GOL-31** static-Nanite tree variant + **GOL-173** tree sway; sound polish + wind-coupling (Polish epic); **GOL-171**/**GOL-172** course layer-data audit + pipeline overlay (mac); **GOL-170** 3D grass; **GOL-169** night; **GOL-27** range R4. Siblings: **GOL-93** ball physics, **GOL-94** course authoring. Other: GOL-152 rename, GOL-153/154, GOL-158/159 (beta).
