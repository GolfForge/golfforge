...
- **Latest:** **GOL-140 — Settings overlay redesign + reusable segmented/toggle controls** (2026-06-02, Windows). Fourth child of the GOL-137 UI-elevation epic. Rebuilt `USettingsMenu` into the GolfForge design (left rail Graphics/Audio/Gameplay/Controls/Credits + `UWidgetSwitcher` sections + label·description·control rows) on the GolfUITheme atoms, **keeping the same public API + the real `GolfDisplaySettings` backing** (HUD wiring untouched). New reusable `UI/SegmentedControl` + `UI/ToggleSwitch` atoms (the segmented one is what the setup wizard reuses) + `GolfUITheme` `StyleSlider`/`StyleComboBox` + button content-padding. Graphics wires the real settings (Resolution + Render-scale dropdowns, segmented Window/Quality/Upscaler) preserving the upscaler→render-% carry-over + borderless lock; Audio/Gameplay/Controls are disabled "Coming soon" rows. In-game **both Esc and Z open Settings** (which carries the Main Menu + Quit shortcuts) — removed the direct-to-main-menu dev path; Main Menu/Quit hidden when opened from the menu. Commit `c75eb6f` (Live-Coding-verified; 2 expected C4996 combo-deprecation warnings). **GOL-137: 4/12 children done (138/139/150/140) — history in `docs/worklog.md`.**
- **Active focus:** (1) **GOL-137 UI elevation** (High) — next children: the **round-setup wizard (GOL-141/142/143)** (reuses the GOL-140 `USegmentedControl`/`UToggleSwitch` atoms + the `label·desc·control` row pattern) or **GOL-151** (Lucide icon font — retires the `→`/`↵` mono stopgaps), then in-round HUD (GOL-144/145). Also queued: **GOL-152** (project/exe rename Golfsim→GolfForge — cross-machine). (2) **v0.0.3-alpha release** — Mac cook (`machine/mac`) then tag + GitHub Release. (3) **GOL-49** — `engine/scripts/package.sh` bundling BuildCookRun + the post-cook `courses/` copy.
