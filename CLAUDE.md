...
- **Latest:** **GOL-124 + GOL-125 — Alpha-3 cook-blockers + polish; cooked Win64 build verified end-to-end** (2026-06-02, Windows). `+MapsToCook` for both `PracticeRange` + `GolfForgeDemoBlack` in `DefaultGame.ini`; new shared `Game/CoursePaths.{h,cpp}` with `GolfsimPaths::ResolveCourseDataDir(CourseId)` (editor + cooked candidates, `heightmap.json` sentinel) replacing the duplicated `DeriveRepoRoot` in `Physics/CourseSurface.cpp` + `Round/RoundState.cpp`; courses tree manual `cp -R` into the stage (UAT blocks `DirectoriesToAlwaysStageAsNonUFS` paths above the project root — future `engine/scripts/package.sh` per GOL-49 automates). Tracer flush moved to `URoundTeeUpSubsystem::ApplyBetweenShotTeleport` (tracer persists through flight + settle, clears on between-shot teleport); `r.Shadow.Virtual.MaxPhysicalPages` 4096→8192 in `DefaultEngine.ini`; new "Main Menu" button in `USettingsMenu` via shared `AGolfRangeHUD::ReturnToMainMenu()`. **60/60 tests pass; stage at `Build/Win64/v0.0.3-alpha/`. GOL-112 epic 100% closed (9 children + 2 polish tickets).**
- **Active focus:** (1) **v0.0.3-alpha release** — Mac cook (`machine/mac`) then tag + GitHub Release with both Win64 + macOS zips. (2) **GOL-49** — committed `engine/scripts/package.sh` that bundles BuildCookRun + the post-cook `courses/` copy. (3) **Post-alpha-3:** practice-mode epics **GOL-73** (closest-to-pin) / **GOL-74** (islands) / **GOL-75** (putting) + ball-physics polish **GOL-39** (coefficient/spin-back tuning, blocked on LM session data).
