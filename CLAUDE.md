...
- **Latest:** **GOL-164 water material polish** (2026-06-05, Windows). New `engine/scripts/build_water_material.py` upgrades `M_GolfsimWater` from flat blue tint to a real pond: ripple via **per-pixel animated normals** (two panned layers of a procedurally-generated tiling `T_WaterNormal`, drifting along the shared `MPC_GolfWind`) — deliberately not WPO, since the flat low-poly water mesh has no verts and WPO would fight ball-water collision; reflections via Lumen (already on) + low Roughness + Fresnel; `DepthFade` shore-foam shallows; deep pond green-blue tint. All knobs are params on a `MIC_GolfsimWater` (live Details-panel tuning, GOL-163 grain), re-assigned to the 10 live `Water_*` actors. Procedural normal generated off-engine with stdlib (no Fab). Verified: compiles clean, params resolve, 10 actors re-skinned. **Persistent** — operator saves `GolfForgeDemoBlack.umap`. Caustics split to **GOL-175**.
- **Active focus (alpha-3, GOL-160 vibe pass):** flag (GOL-165) + materials (GOL-163) + atmosphere (GOL-161) + water (GOL-164) done; props (GOL-168) range first-pass done, remainder deferred. Remaining children: **GOL-166** ambient SFX, **GOL-167** tree species, **GOL-168** remainder + **GOL-175** water caustics (Fab/Light-Function); optional GOL-163 **P4** per-region noise. **Deferred:** **GOL-173** tree sway (later epic); **GOL-171** course layer-data audit (mac) + **GOL-172** pipeline sanity-overlay (mac) + any engine fix for the splat→landscape mapping; **GOL-170** 3D grass; **GOL-169** night; **GOL-27** range R4. Siblings: **GOL-93** ball physics, **GOL-94** course authoring. Other: GOL-152 rename, GOL-153/154, GOL-158/159 (beta).
