...
- **Latest:** **GOL-75 putting practice mode — logic DONE (2026-06-10, Windows)** — Phase A of the epic remainder (the *physics* was finished earlier today; the dedicated putting-green map is Phase B). New **`EPracticeMode::Putting`** drill that **extends the CTP scaffold**: a pin spawns at a random short distance, the player putts from the tee with the putter, and **every putt is a counted stroke** — holing out within the gimme scores **putts-to-hole** (a `DistanceToPin` toggle scores the first putt by rest distance instead). Pure core adds `EScoreMode` + `FCtpConfig::Score` + `MakePuttingDefaults()` (5–30 ft, hole-out, 1-ft gimme) + `MetersPerFoot` (SI stored, **feet at the UI boundary**). Subsystem `StartPuttingSession`/`RecordHoleOut`; HUD putt-counting flow + feet/putts scoreboard (reuses `IsWithinGimme` + putt-teleport); panel feet min/max (5-ft steps) + scoring toggle + a **disabled line-preview seam** (deferred — needs an aim solver); the "Putting" practice card is live. Reuses `FPracticeShotScoredEvent` (`source=practice-putting`), no schema change. New `Golfsim.Practice.PuttingDefaults`+`PuttingScoring` tests; suite **92/92**. Break magnitude still provisional (LM-gated, GOL-195).
- **Active focus — alpha-2 gameplay foundation.** The alpha-3 visual vibe pass is done; now make it *play* like a course. **(1) Bunkers — GOL-34 DONE. (2) Ball physics — GOL-93:** GOL-38 multi-bounce + GOL-39 cross-surface/spin-back + GOL-196 terrain-aware bounce DONE; remaining is **GOL-195** (per-surface coefficient calibration vs Square Omni LM data + promote `FSurfaceRoll` to a tunable UDataAsset/UDeveloperSettings) — **LM-gated, deferred**. **(3) Practice mode — GOL-73 closest-to-pin DONE** (carry + flat-green putt-out). **(4) Pin positions — GOL-191/192 DONE** (drop the course disc + Static/Random/Tournament pins from `green.geojson`; gimme-ring polish). **(5) Putting — GOL-75:** physics DONE (fall-line roll, break magnitude provisional/LM-gated GOL-195) + **putting practice mode logic DONE** (Putting drill extends the CTP scaffold: feet settings, scored putts-to-hole / distance-to-pin, runs on existing greens). **Remaining = Phase B:** a dedicated `PuttingGreen.umap` built from a real famous contoured green via the LIDAR+OSM pipeline (site gated on data coverage) so break plays on genuine slopes — plus the deferred line-preview (aim solver). Next practice ticket: islands **GOL-74**. **Parallel — UI cleanup:** tracer UI (GOL-107), reskin secondary panels onto `GolfUITheme` (GOL-155), round-setup pills (GOL-153).
