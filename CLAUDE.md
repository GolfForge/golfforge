...
- **Latest:** **GOL-142 — Round Setup wizard: Format step + Holes/gimmes wired live** (2026-06-03, Windows). Sixth child of the GOL-137 UI-elevation epic. Built the Format step (Holes / Game type / Turn order / Hole-out) and — beyond the ticket's "disabled seams" scope — **wired Holes + gimmes into gameplay**. New `Round/RoundConfig.h` (`FRoundConfig`, rides on `FRoundState` so it survives the deferred map-load). Pure `GolfsimRound::SelectHoles` subsets the schedule (Full18/Front9/Back9/Custom) — the state machine was already subset-agnostic, so Front 9 ends after 9, etc.; `URoundSubsystem::StartRound` gained a defaulted `FRoundConfig` param. Gimmes: `GolfsimRound::EffectiveGimmeRadiusFt` loosens (never tightens) the auto-hole tolerance; `URoundHoleOutSubsystem` reads it. `USegmentedControl` extended (sub-labels + per-option-disable + content-sizing — Fill-equal was clipping long labels); new `UI/OptionCard` (game/turn cards, **seams** — Stroke/Play-it-out live, rest GOL-69). Tests `Golfsim.Round.SelectHoles` + `GimmeRuleRadius` (13/13 green). Commit `3286740` (full-build + tests + PIE verified). **GOL-137: 6/12 children done (138/139/150/140/141/142) — history in `docs/worklog.md`.**
- **Active focus:** (1) **GOL-137 UI elevation** (High) — next child: **GOL-143** (round-setup **Step 3 Players** — count + per-player rows + round summary, then Tee Off; reuses the wizard chrome + `USegmentedControl`), then in-round HUD (GOL-144/145). Also queued: **GOL-151** (Lucide icon font), **GOL-152** (project/exe rename Golfsim→GolfForge — cross-machine), **GOL-153** (stepper pill repaint). (2) **v0.0.3-alpha release** — Mac cook (`machine/mac`) then tag + GitHub Release. (3) **GOL-49** — `engine/scripts/package.sh` bundling BuildCookRun + the post-cook `courses/` copy.
