...
- **Latest:** **All 5 Bethpage demo courses live (2026-06-09, Windows).** Added Blue/Red/Green/Yellow alongside Black as standalone playable tracks, selectable from the course picker (validated in PIE). Pipeline generated each from a per-track **2016m-square real-scale bbox** (exclusive masks via the GOL-171 resolver, all 18 holes); levels built by duplicate-Black + re-import + scripted vibe pass (water + PCG trees + birds-only SFX). Engine: hoisted the course-id↔level map into a shared `Course/CourseLevelMap.h` (kills the duplicate-table TODO), registered all 4 in `CourseRegistry`, added to `MapsToCook`. Parameterized the per-course scripts (one `COURSE_ID`). **Gotcha:** set landscape Z scale **by hand** in Details — Python `set_actor_scale3d` floats the PCG trees (see cookbook). Beds (wind/murmur) left off for now per request; **Blue still has a wind bed** (built before that call).
