...
- **Latest:** **0.0.8-alpha HUD upgrade (2026-06-19, Windows).** GOL-149 one-key **HUD density cycle** (`V`: Full = telemetry card + control bar / Compact = a new **2-column left metrics tower** / Hidden = clean for screenshots; persisted via `GolfDisplay::Read/WriteHudDensity`, enforced each Tick by `AGolfRangeHUD::ApplyHudDensity`). Tower adds apex/descent/hang (from `FBallTrajectory`) + a conditional club-delivery block. GOL-155 (partial) **shot-history reskin** onto `GolfUITheme`. **GSPro club-data capture** (`ClubData` → `ClubSpeedMps/AttackAngleDeg/ClubPathDeg/FaceToTargetDeg` on the shot events, shown only when a connector sends it). Suite **107/107** on UE 5.8; committed (`7dabcc2` + tower rework), **not yet cooked** — 0.0.8-alpha pending two more docket items. Also recent: MLM2PRO validated via BT-APP (GOL-181, docs). **5.8 carryover open-items:** MCP automation restored via the `GolfsimAgentTools` Python toolset on Epic's `ModelContextProtocol` (`.mcp.json` `unreal-mcp` HTTP `127.0.0.1:8000/mcp`; subclassing `ToolsetDefinition` does NOT auto-register — call `Registration([cls]).register()`); upscalers DLSS/Streamline/XeSS disabled (need 5.8 Fab builds before release; TSR meanwhile); deprecation cleanup pending (`OnPostEngineInit`, `UComboBoxString` getters, `GetMovementBase`, `Landscape::HasLayersContent`). Engine path `C:\Program Files\Epic Games\UE_5.8` (5.7 at `D:\ue57` fallback). Migration recipe in `docs/ue5-cookbook.md`.
