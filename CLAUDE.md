...
- **Latest:** **GOL-73 closest-to-pin practice mode DONE (2026-06-09, Windows).** First slice of the CTP epic: a pin spawns at a random distance, the player hits, the system scores by distance-from-pin, repeat. New pure-SI core `GolfsimPractice::` (`Practice/PracticeMode.{h,cpp}`, mirrors `GolfsimRound::`) — `NextPin`/`ScoreDistanceM`/stats, 6 headless tests. `UPracticeModeSubsystem` owns mode/config/session + publishes `practice.shot_scored`; `AGolfRangeHUD` orchestrates (side-offset pin via extended `ApplyPinDistance`, score-on-settle off the `Tick` hook, **flat-green putt-out** reusing the putter + `GolfsimRound::IsWithinGimme`, 2 s next-pin gap). UI: Mode dropdown + CTP settings cluster (Min/Max 5-yd, side, putt-out) + This/Best/Avg/Shots scoreboard on `GolfRangePanel`; `FShotHistoryEntry.Mode` tag; `golfsim.Practice.CTP/Free/End/Status` console. Realistic putt-out *break* deferred (GOL-75/191). `Golfsim.Practice` 6/6 + full suite 83/83 green.
