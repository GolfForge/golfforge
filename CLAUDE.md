...
- **Latest:** **GOL-163 surface material upgrade DONE (2026-06-09, Windows).** Closed the last two "Done when" boxes: **P4 per-region color variation** (`_region_variation()` — low-freq world-position `MaterialExpressionNoise`, GRADIENT_ALU = **procedural, no extra sampler**; `1+noise*amount` into the albedo so each surface's tone drifts naturally, no flat-tile look on flyovers; live MIC knobs `<Layer>_VarAmount`/`_VarScale` on Fairway/Green/Rough/Tee + subtle Bunker) + a **distinctness re-tune** (Green cooler/wetter, Tee yellower, Rough browner, **Bunker whitened off raw desert sand** to Bethpage cream). Helper ported verbatim into `build_range_material.py` (builders stay diffable). Both materials rebuild clean end-to-end; MICs rebuilt + landscapes rebound; validated distinct in PIE. Noise 5.7 Python gotchas (`noise_function` not `function`; empty-string Position pin) in the cookbook.
- **Active focus — alpha-2 gameplay foundation.** The alpha-3 visual vibe pass is done; now make it *play* like a course, not just look like one. **(1) Bunkers — GOL-34 (Urgent, NEXT):** turn flat sand polygons into believable traps via a heightmap depression + raised green-side lip (Approach A: pipeline emits `bunker.geojson`, then `engine/scripts/build_bunker_depressions.py` sculpts the landscape — idempotent off the baseline `heightmap.png`; UE5.7 heightmap edits are partially bound, fall back to modify-PNG-and-re-import). **Ship on the current physics** (course lie-detection GOL-40 already done); tune bunker feel later. **(2) Ball physics — GOL-93:** GOL-38 multi-bounce/skip + GOL-39 per-surface coefficients / green spin-back / cross-surface / `UDataAsset` (pure-SI, TDD). **(3) Practice mode — GOL-73 closest-to-pin** once physics+bunkers land (islands GOL-74 / putting GOL-75 deferred — putting waits on green geometry GOL-191). **Parallel — UI cleanup:** tracer UI (GOL-107), reskin secondary panels onto `GolfUITheme` (GOL-155), round-setup pills (GOL-153).
