...
- **Latest:** **GOL-148 — Scorecard reskin + GOL-137 epic closeout** (2026-06-03, Windows). Twelfth and final child — **the GOL-137 UI-elevation epic is complete (12/12: 138/139/150/140/141/142/143/144/145/146/147/148)**. Reskinned `ScorecardPanel.{h,cpp}` onto `GolfUITheme` (the last pre-theme spec screen): blurred scrim, fixed-width `Bg1` glass card, eyebrow+title, faint-mono columns, mono rows with **vs-par colors** (birdie→Accent / par→Text / bogey→Caution / double+→DangerText), divider + emphasized TOTAL, accent "Back to menu"; data API unchanged. Added `+DirectoriesToAlwaysCook=(Path="/Game/UI")` to `DefaultGame.ini` so the theme's `LoadObject`'d fonts + gradient materials stage in a cook. Fidelity sweep: scorecard was the only token drift. Full-build + 63/63 tests + PIE verified. History → `docs/worklog.md`.
- **Active focus:** (1) **v0.0.4-alpha release** — combined **Win + Mac cook** now that the whole UI epic has landed, then tag + GitHub Release; gated on **GOL-49** (`engine/scripts/package.sh` = BuildCookRun + post-cook `courses/` copy) and a cook-time check that the new `/Game/UI` fonts/materials stage. (2) **Remaining UI polish/follow-ups:** **GOL-151** (Lucide icon font — retires placeholder leave/control-bar/condition/avatar icons), **GOL-152** (rename Golfsim→GolfForge, cross-machine), **GOL-153** (stepper pill repaint), **GOL-154** (live weather → conditions wind/temp), plus new follow-ups: **dev/secondary panel reskins** (cheat sheet, shot history, previous-sessions, manual-shot dialog) and a **reduced-motion a11y setting**.
