...
- **Latest:** **GOL-165 fluttering GolfForge-branded pin flag + shared wind source** (2026-06-05, Windows, commit `68ababb`). `AGolfPinActor` flag → subdivided `UProceduralMeshComponent` grid driven by **`M_FlagWind`** (two-sided WPO flutter, `wave=sin(Time*Freq - U*WaveDensity)` along world `WindDirection`). BaseColor = branded **`T_GolfForgeFlag`** (golden tee + "GolfForge" wordmark), UV fixed via vertical flip + **`TwoSidedSign`** so it reads right from both faces. New shared **`MPC_GolfWind`** (`WindDirection`+`WindStrength`) = the GOL-154 wind hook. Built by `engine/scripts/build_flag_wind_material.py`; `ProceduralMeshComponent` added to `Build.cs`. **Note:** the flag shows on **cold editor start** (the `static FObjectFinder` resolves `M_FlagWind` only at startup; a session that built it mid-run falls back to single-sided BasicShapeMaterial → invisible-in-PIE artifact, not a bug). **Tree sway split to GOL-173** (PCG skinned-mesh instances don't evaluate the PVE foliage WPO) — deferred to a later epic. Recipes → `docs/ue5-cookbook.md`.
- **Active focus (alpha-3, GOL-160 vibe pass):** flag (GOL-165) + materials (GOL-163) done. Remaining children: **GOL-164** water polish, **GOL-166** ambient SFX, **GOL-167** tree species, **GOL-168** props; optional GOL-163 **P4** per-region noise. **Deferred:** **GOL-173** tree sway (later epic); **GOL-171** course layer-data audit (mac) + **GOL-172** pipeline sanity-overlay (mac) + any engine fix for the splat→landscape mapping; **GOL-170** 3D grass; **GOL-169** night; **GOL-27** range R4. Siblings: **GOL-93** ball physics, **GOL-94** course authoring. Other: GOL-152 rename, GOL-153/154, GOL-158/159 (beta).
