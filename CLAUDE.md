...
- **Latest:** **GOL-162 3D fairway grass — Grass Detail quality toggle** (2026-06-04, Windows). Grass was **already wired** on the course (`build_course_material.py:431`, Fairway-only, cull ~35 m), so GOL-162 shipped the perf toggle, not a re-wire: a **Grass Detail Off/Low/High** setting (`FGolfDisplaySettings.GrassDetailLevel` → `grass.densityScale`/`grass.CullDistanceScale`, confirmed cvars), Off = the KBG texture fallback, persisted in `[GolfForge.Graphics]`, **`#if PLATFORM_MAC` default Low** (Win High), with a boot-time apply via a new `FGolfsimModule` `OnPostEngineInit` hook. Plus `golfsim.SetGrassDetail`, a Settings→Graphics control, and a clamp unit test (build green, `Golfsim.Settings` passes). Decisions: Fairway-only (greens skipped), cull stays ~35 m. **Nanite finding:** KBG meshes are Nanite-enabled + opaque = already the efficient case; Nanite Foliage's tree-focused wins route to GOL-167/GOL-31. **Remaining: PIE visual + perf pass (user-driven).** Recipe → `docs/ue5-cookbook.md`.
- **Active focus (alpha-3, GOL-160 vibe pass):** GOL-162 code done (PIE perf check pending). Next in epic order: **GOL-163** surface materials (distinct fairway/rough/green/tee/bunker in `build_course_material.py`), then **GOL-164** water, **GOL-165** flag/tree wind, **GOL-166** ambient SFX, **GOL-167** tree species (Nanite Foliage pays off here), **GOL-168** props. Alpha-3 siblings (no conflict): **GOL-93** ball physics, **GOL-94** course authoring. Other Windows follow-ups: GOL-152 module rename, GOL-153/154, **GOL-169** night, GOL-27 range R4 deep-tune, GOL-158/159 (beta).
