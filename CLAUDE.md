...
- **Latest:** **GOL-209 round-HUD minimap + GOL-210 top-gradient removal DONE (2026-06-12, Windows; user-validated in PIE).** The hole-map card is a real minimap: new pipeline stage `pipeline/build_minimap.py` bakes `courses/<id>/minimap.png` (layer-mask palette + water + heightmap hillshade, splatmap georeference; regenerated for all 6 full courses, LFS). Engine: `UI/HoleMapProjection` (pure affine + framings + green slope grid; 5 new tests, suite 103/103), `Game/MinimapTexture` (first runtime PNG→`UTexture2D`), `UI/HoleMapView` (**repo's first NativePaint widget** — see the new cookbook section for the Slate traps). Hole-axis-up basemap + ball/pin/aim/yard-arcs, wheel zoom, click-to-aim (`OnAimAt`→`SetControlRotation`); GREEN tab = slope heatmap + downhill break arrows from a per-hole trace grid + pulsing ball ring; auto-tab follows play (HOLE on hole.start, GREEN when the ball reaches the green polygon). Three persisted sizes (chip → 280 card → 480 large), **M cycles them; the manual-shot dialog moved M→N**. Swing meter now resets 2 s after the ball settles. GOL-210: the 300px full-width top gradient is gone.
- **Active focus — alpha-2 gameplay foundation.** The alpha-3 visual vibe pass is done; now make it *play* like a course. **(1) Bunkers — GOL-34 DONE. (2) Ball physics — GOL-93:** GOL-38 multi-bounce + GOL-39 cross-surface/spin-back + GOL-196 terrain-aware bounce + GOL-206/207 green-roll correctness DONE; remaining: **GOL-208** (Penner spin-vector ground model, Medium) then **GOL-195** (per-surface coefficient calibration vs Square Omni LM data + promote `FSurfaceRoll` to a tunable UDataAsset/UDeveloperSettings) — **LM-gated, deferred**. **(3) Practice mode — GOL-73 closest-to-pin DONE** (carry + flat-green putt-out). **(4) Pin positions — GOL-191/192 DONE** (drop the course disc + Static/Random/Tournament pins from `green.geojson`; gimme-ring polish). **(5) Putting — GOL-75 + GOL-199 DONE:** physics + practice-mode logic + **putt-on-a-real-green on the OldAndre links (St Andrews Old Course, whole ~2 km map)** all shipped — cup capture, ball-at-address, lie-correct re-putts, banner, real break on the 18th. **GOL-204 trees-to-world-up DONE** + **GOL-205 OldAndre full playable links DONE** + **GOL-206/207 green-roll + spin-check fixes DONE.** **(6) Round HUD — GOL-209 minimap + GOL-210 gradient DONE.** **NEXT (prioritized, alpha-release suspects): GOL-203** (elevate putting: UMG hole-out toast+SFX, putt camera, ground tracer, capture tuning), **GOL-202** (practice-UI gating). Also: **GOL-201** famous-greens tour (OldAndre has 84 greens to hop), **GOL-200** break-aware line preview (the minimap's slope-grid + projection infra is reusable for it). Next practice ticket: islands **GOL-74**.
