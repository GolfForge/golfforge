...
- **Latest:** **Hide springbok connector + DLSS Frame Generation toggle (GOL-181/GOL-189)** (2026-06-07, Windows). Springbok/MLM2PRO dropdown entry **commented out** in `LaunchMonitorManager` (stale + needs GSPro APIv1 connect window; quirks kept in `GSProConnectDriver` for re-enable) — shipping connectors are OpenFlight + GSPro Connect + Square Golf. Added a **family-aware DLSS Frame Generation** setting (`GolfDisplaySettings` `FrameGenMode` + `SupportedFrameGenModes()` from the GPU's `GetSupportedDLSSGModes()`, `SetDLSSGMode` apply, `golfsim.SetFrameGen`, Settings row, Reflex forced on with FG; `Build.cs` `GOLF_WITH_DLSSG` gates the optional plugin module so Mac still builds). Build green, `Golfsim.Settings.FrameGen` passes. **OPEN GOL-189: FG not generating yet** (`stat DLSSG` Frames Presented = 0 in standalone-from-editor) — likely needs the cooked exe / vsync-cap / Reflex-runtime; toggle ships in 0.0.5 unconfirmed. Before this: range pin overhaul (collar/cup/striped flag/textured green) + mow-stripe checkerboard; **v0.0.5-alpha** cut (ProjectVersion 0.0.5-alpha; README/docs updated). `AGolfPinActor` got a darker textured **collar ring**, a **hole cup**, and a white/black **striped flagstick** (`M_FlagPole`); `M_GolfGreen` is now a **textured** round disc (procedural `T_TurfGrain` normal + `Color`/`Roughness`/`GrainTiling`/`GrainShade` params, green color = material default so it's script-tunable, no rebuild). Range mow stripes reworked from blobby sine-product to a true **checkerboard** (sharpened square-wave bands, sign product; new `_StripeSharpness`); on-turf floor numbers disabled (`PLACE_GROUND_NUMS=False`), **GOL-28 closed**. **ProjectVersion → 0.0.5-alpha**; README/worklog/cookbook updated for the GOL-160 facelift + GSPro-Connect story. Build green; verified in PIE. Follow-ups: club/face-impact **GOL-187**; remaining connectors **GOL-181** (P4); cook+tag the 0.0.5 binaries (user).
