...
- **Latest:** **GOL-73 closest-to-pin practice mode DONE (2026-06-09, Windows).** First slice of the CTP epic: a pin spawns at a random distance, the player hits, the system scores by **approach distance-from-pin** (putts never counted), repeat. New pure-SI core `GolfsimPractice::` (`Practice/PracticeMode.{h,cpp}`, mirrors `GolfsimRound::`) — `NextPin`/`ScoreDistanceM`/stats, 6 headless tests. `UPracticeModeSubsystem` owns mode/config/session + publishes `practice.shot_scored`; `AGolfRangeHUD` orchestrates (side-offset pin via extended `ApplyPinDistance`, score-on-settle off the `Tick` hook, unscored **flat-green putt-out** reusing the putter + `GolfsimRound::IsWithinGimme`, 2 s next-pin gap). **Entry: the Practice main-menu tile** opens `UI/PracticeSetup` (drill card picker, reuses `UI/OptionCard`); the Range tile stays plain free-fire. In-drill HUD on `GolfRangePanel`: PIN target + This/Best/Avg/Shots scoreboard + End-drill. Range UX: `C` toggles camera, follow cam resets ball+frame to tee after 3 s idle. `FShotHistoryEntry.Mode` tag; `golfsim.Practice.*` console. Realistic putt-out *break* deferred (GOL-75/191); `DrawDebugLine` result line → GOL-197. `Golfsim.Practice` 6/6 + full suite 83/83 green.
- **Active focus — alpha-2 gameplay foundation.** The alpha-3 visual vibe pass is done; now make it *play* like a course. **(1) Bunkers — GOL-34 DONE. (2) Ball physics — GOL-93:** GOL-38 multi-bounce + GOL-39 cross-surface/spin-back DONE; remaining is **GOL-195** (per-surface coefficient calibration vs Square Omni LM data + promote `FSurfaceRoll` to a tunable UDataAsset/UDeveloperSettings) — **LM-gated, deferred**. **(3) Practice mode — GOL-73 closest-to-pin DONE** (carry + flat-green putt-out; realistic putt-out *break* deferred → GOL-75/GOL-191). Next practice tickets: islands **GOL-74**, putting **GOL-75** (waits on green geometry GOL-191). **Parallel — UI cleanup:** tracer UI (GOL-107), reskin secondary panels onto `GolfUITheme` (GOL-155), round-setup pills (GOL-153).
