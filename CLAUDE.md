...
- **Latest:** **GOL-178 GSPro Open Connect V1 server driver** (2026-06-06, Windows). New `UGSProConnectDriver : ULaunchMonitorDriver` (`Drivers/GSProConnectDriver.{h,cpp}`) makes GolfForge *the GSPro server*: a raw TCP listener on `127.0.0.1:921` (newline-delimited JSON, no auth) that the community LM connectors dial — so one driver inherits six launch monitors (MLM2PRO, Mevo+, SkyTrak, Garmin R10, Square Omni, GC2) with no per-LM code, no hardware for bring-up, and no websockets (sidesteps GOL-36). Mirrors `UOpenFlightDriver`: pure static `ParseShot` (BallData→`FShotTakenEvent`, reusing the mph→m/s + TotalSpin/SpinAxis decomposition), a dedicated `FRunnable` listener marshaling shots to the game thread over an SPSC `TQueue`+`FTSTicker` (publish must be game-thread), `{Code:200}` acks + `{Code:201}` player push, last-wins single client. Opt-in (openflight stays default active); `[LaunchMonitor.GSProConnect] Port=921`. Clean UE 5.7 build; all 7 `Golfsim.GSProConnect.*` tests pass. Live TCP + real-connector validation (squaregolf-connector mock) run in parallel; full 6-connector matrix is **GOL-181**.
