...
- **Latest:** **GOL-166 ambient SFX layer** (2026-06-05, Windows). Both silent maps now have outdoor ambience. New `engine/scripts/place_ambient_sfx.py` (mirrors `place_range_props.py`): imports CC0 BigSoundBank `.ogg`s (`engine/Golfsim/Content/Audio/Ambient/`, credited in `ATTRIBUTION.md`) to looping `USoundWave`s (loop flag is hidden from `dir()` but settable by name; no SoundCue needed/possible from Python), routes them through `SC_Ambient`, and places `AAmbientSound` bird zones **anchored to sampled PCG tree instances** (angular spread on course, down-lane on range) with an `SA_AmbientBird` attenuation tuned by ear (100% within 15 m, 50% at 70 m, silent ~125 m; vol 0.90). Shipping **birds-only** (`AMBIENT_BEDS=False`; wind + distant-traffic murmur beds kept behind the toggle after the village-murmur clip's baked-in birdsong leaked through the *global* bed). **Gameplay gate** in `AGolfRangeHUD::UpdateAmbientPlayback` (C++): forces `bAutoActivate=false`+`Stop()`, then `FadeIn/FadeOut` on `!InputGated()`, so ambience is silent under the menu/course-select and fades in only in actual gameplay. Verified in PIE; both umaps saved. Wind-volume coupling deferred to GOL-154.
- **Active focus (alpha-3, GOL-160 vibe pass):** flag (GOL-165) + materials (GOL-163) + atmosphere (GOL-161) + water (GOL-164) + tree species (GOL-167) + ambient SFX (GOL-166) done; props (GOL-168) range first-pass done, remainder deferred. Remaining children: **GOL-168** remainder + **GOL-175** water caustics (Fab/Light-Function); optional GOL-163 **P4** per-region noise. **Deferred:** **GOL-31** static-Nanite tree variant (asset to source) + **GOL-173** tree sway (later epic); sound polish + wind-coupling under the new **Polish / nice-to-have epic** (see Linear); **GOL-171** course layer-data audit (mac) + **GOL-172** pipeline sanity-overlay (mac) + any engine fix for the splat→landscape mapping; **GOL-170** 3D grass; **GOL-169** night; **GOL-27** range R4. Siblings: **GOL-93** ball physics, **GOL-94** course authoring. Other: GOL-152 rename, GOL-153/154, GOL-158/159 (beta).
