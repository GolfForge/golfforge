...
- **Latest:** **GOL-146 — Swing meter: glass reskin + reposition clear of the control bar** (2026-06-03, Windows). Tenth child of the GOL-137 UI epic; the deferred swing-meter half of GOL-145. **Visual reskin + reposition only** — the ticket's data-reconciliation half (adopt the JS prototype's CLUBS/tunables/result-math) was **intentionally dropped per the user**: the prototype has no ball-flight physics (it fakes shots with lookup tables), so it's a *look* reference only and can't dictate engine behavior — `KeyboardSwingComponent`, `FConfig` tunables, `GBag`, `ResolveShot`, and the GOL-122 difficulty profiles are untouched. Rebuilt `USwingMeterWidget` against `GolfUITheme` to match `06-hud-gamemode.png`: glass panel, header eyebrow + green `GAME MODE · KEYBOARD` pill, width-driven power fill + live `NN%`, accuracy track with an always-visible green 80–90 zone + moving marker, per-bar readouts, `[Space]`-keycap prompt. New `OnPowerLocked`/`OnAccuracyResult`/`ResetMeter` API; HUD drives qualitative `Pure`/`Push R`/`Pull L` feedback (offline yardage stays physics-derived in the readout). Lifted clear of the control bar via a **negative `Offset.Top`** (autosize canvas slots ignore `Offset.Bottom` — cookbook). Full-build + 63/63 tests + PIE verified. **GOL-137: 10/12 children done (138/139/150/140/141/142/143/144/145/146) — history in `docs/worklog.md`.**
- **Active focus:** (1) **GOL-137 UI elevation** (High) — next: **GOL-147** (leave/quit modal — wires the round panel's Menu guard), then **GOL-148** (scorecard reskin + epic closeout). Also queued: **GOL-151** (Lucide icon font — retires the placeholder control-bar/condition/avatar icons), **GOL-152** (rename Golfsim→GolfForge, cross-machine), **GOL-153** (stepper pill repaint), **GOL-154** (live weather → conditions wind/temp). (2) **v0.0.3-alpha release** — Mac cook (`machine/mac`) then tag + GitHub Release. (3) **GOL-49** — `engine/scripts/package.sh` bundling BuildCookRun + the post-cook `courses/` copy.
