...
- **Latest:** **GOL-34 bunker depressions DONE + PIE-validated (2026-06-09, Windows).** New **pipeline-side** `pipeline/build_bunker_depressions.py` (numpy/PIL — *not* engine/scripts stdlib, since UE5.7 can't import heightmaps via Python anyway and reusing `build_splatmap.lonlat_to_pixel` aligns the sculpt pixel-perfect with `splat_bunker.png`) sculpts a depressed sand floor + raised lip into the heightmap via an exact pure-numpy Felzenszwalb distance transform. Writes a **sibling `heightmap_bunkers.png`** (leaves `heightmap.png` pristine → byte-identical idempotent re-runs); encoding preserved so the UE re-import is **heightmap-only, no Z change**. Generated + re-imported for all 5 demo courses (~50 cm drop / 25 cm lip); 12 new tests, full suite 61 green. PIE: ball hit in rolls and rests inside. Reads well even at the ~1 m/px grid. Green-side lip bias → GOL-194; sand *friction* is GOL-39 (ball still rolls like grass — by design).
- **Active focus — alpha-2 gameplay foundation.** The alpha-3 visual vibe pass is done; now make it *play* like a course, not just look like one. **(1) Bunkers — GOL-34 DONE.** **(2) Ball physics — GOL-93 (NEXT):** GOL-38 multi-bounce/skip + GOL-39 per-surface coefficients / green spin-back / cross-surface / `UDataAsset` (pure-SI, TDD) — note GOL-39 is what makes sand actually drag (bunkers currently use uniform rolling friction; lie detection GOL-40 already classifies sand). **(3) Practice mode — GOL-73 closest-to-pin** once physics lands (islands GOL-74 / putting GOL-75 deferred — putting waits on green geometry GOL-191). **Parallel — UI cleanup:** tracer UI (GOL-107), reskin secondary panels onto `GolfUITheme` (GOL-155), round-setup pills (GOL-153).
