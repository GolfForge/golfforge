...
- **Latest:** **GOL-163 surface material upgrade — distinct, param-tunable surfaces + mowing stripes** (2026-06-05, Windows). Reworked `M_GolfsimCourse`: per-surface tints (VectorParam `<name>_Tint`) + roughness (ScalarParam `<name>_Rough`) + mowing-stripe params, all driving a new **`MIC_GolfsimCourse`** (Material Instance on the landscape) for **live tuning, no rebuild**. Distinct Fab textures: Fairway+Green→`Cut_Grass`, Trees→`Mossy_Forest_Floor`, Tee→Lawn. Mowing stripes = world-aligned albedo band. `BUILD_GRASS=False` grass-free guard. Commits `cbebe92`/`c078532`/`5a2cbb2` (+ `a83c0f1` GOL-108 landscape weight re-import). **Data caveat:** materials render only where landscape layers resolve — most holes lack fairway paint; the re-import fixed edge holes but a **splat↔landscape mapping/data issue remains** → **GOL-171** (audit, mac) + **GOL-172** (pipeline sanity-overlay) deferred to end of epic. Params+MIC+stripe recipe → `docs/ue5-cookbook.md`.
- **Active focus (alpha-3, GOL-160 vibe pass):** GOL-163 materials done (P1/P2/P3); optional **P4** per-region noise remains. Next children: **GOL-164** water polish, **GOL-165** flag/tree wind, **GOL-166** ambient SFX, **GOL-167** tree species, **GOL-168** props. **End-of-epic cleanup (deferred):** **GOL-171** course layer-data audit (mac) + **GOL-172** pipeline sanity-overlay (mac) + any engine fix for the splat→landscape mapping; **GOL-170** 3D grass; **GOL-169** night; **GOL-27** range R4. Siblings: **GOL-93** ball physics, **GOL-94** course authoring. Other: GOL-152 rename, GOL-153/154, GOL-158/159 (beta).
