...
- **Latest:** **GOL-203 elevate-the-putting-experience DONE (+ GOL-202/198 practice-UI gating) (2026-06-13, Windows; user-validated in PIE "this looks perfect").** Putting now reads real: themed UMG `UI/HoleOutToast` (replaces the `DrawHUD` banner; also fires score-flavored on round `hole.complete` + CTP putt-outs), cup-drop sink animation + `SW_CupDrop` SFX (**first CC-BY asset** — Freesound/AGFX, ATTRIBUTION.md updated), hole-out resolves the **instant cup capture is detected mid-roll**, auto **putt camera** (terrain-Z-clamped for links swales, rides arrow-key aim with a live ground aim line, hides the FP pawn, third "Putt" dropdown entry), ground-hugging putt tracer, and an **in-world flowing-dots break grid** (`Range/GreenBreakGridActor` + `UI/GreenFlowTexture` pure encoder w/ 3 tests + `engine/scripts/build_green_flow_material.py` two-phase flow-map `M_GreenFlow`; `BuildGreenSlopeGrid` extracted from `PushHoleMapStatic` so minimap + overlay share one trace burst; **G** toggles). Course greens hole out via real cup capture only (no gimme); GOL-202/198 = mode-gate the range dev controls (`UpdateInRoundHud` was re-showing them every tick) + putt-length-in-feet readout; focus traps fixed (spinboxes/map buttons/segmented tabs return focus to game). Suite 106/106.
- **Active focus — alpha-2 gameplay foundation.** The alpha-3 visual vibe pass is done; now make it *play* like a course. **(1) Bunkers — GOL-34 DONE. (2) Ball physics — GOL-93:** GOL-38 multi-bounce + GOL-39 cross-surface/spin-back + GOL-196 terrain-aware bounce + GOL-206/207 green-roll correctness DONE; remaining: **GOL-208** (Penner spin-vector ground model, Medium) then **GOL-195** (per-surface coefficient calibration vs Square Omni LM data + promote `FSurfaceRoll` to a tunable UDataAsset/UDeveloperSettings) — **LM-gated, deferred**. **(3) Practice mode — GOL-73 closest-to-pin DONE** (carry + flat-green putt-out). **(4) Pin positions — GOL-191/192 DONE** (drop the course disc + Static/Random/Tournament pins from `green.geojson`; gimme-ring polish). **(5) Putting — GOL-75 + GOL-199 DONE:** physics + practice-mode logic + **putt-on-a-real-green on the OldAndre links (St Andrews Old Course, whole ~2 km map)** all shipped — cup capture, ball-at-address, lie-correct re-putts, banner, real break on the 18th. **GOL-204 trees-to-world-up DONE** + **GOL-205 OldAndre full playable links DONE** + **GOL-206/207 green-roll + spin-check fixes DONE.** **(6) Round HUD — GOL-209 minimap + GOL-210 gradient DONE.** **(7) Putting polish — GOL-203 + GOL-202/198 DONE.** **NEXT (prioritized, alpha-release suspects): GOL-211** (real 3D cup — the flat black disc floats on undulating greens; new follow-up from GOL-203), **GOL-200** break-aware line preview (bumped Low→Medium — the GOL-203 slope-grid/flow infra + minimap projection are reusable; show the ideal-putt path that holes out), **GOL-201** famous-greens tour (OldAndre has 84 greens to hop). Next practice ticket: islands **GOL-74**. Deferred putting follow-ups: capture-radius/lip-out tuning, putter-tuned swing meter, session putting stats.
