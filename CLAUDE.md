...
- **Latest:** **v0.0.7-alpha cook-asset fix + GOL-211 (partial) cup-conform + putt aim-line drape (2026-06-13, Windows).** Cooked-build smoke test caught three cook-only bugs (no SFX, gimme ring + break-grid rendering gray-checker): the GOL-148 gotcha again — `M_GimmeRing`/`M_GreenFlow` (`/Game/Materials`) + `SW_CupDrop`/`SW_BallStrike` (`/Game/Audio`) are runtime `LoadObject` loads, dropped from the cook since only `/Game/UI` was always-cooked. Fix: added both dirs to `DirectoriesToAlwaysCook` (config-only), re-cut v0.0.7-alpha. **Anything reached by `LoadObject("/Game/...")` must be smoke-tested in the cooked exe, not just PIE.** Cup disc is now a terrain-draped `UProceduralMeshComponent` (`BuildHoleCup`, per-vertex traces like `BuildGimmeRing`, rebuilt on every pin move via a `RootComponent->TransformUpdated` hook; lift 3cm→0.5cm + `SetCastShadow(false)`) so it lies flush instead of clipping/floating on undulating greens — `GolfPinActor.{h,cpp}` only. The GOL-203 putt aim line now **drapes the terrain** (samples ground Z per 25cm, rides 2cm up) so uphill putts don't bury it. **Real recessed 3D cup deferred** (below-grade geometry needs a runtime landscape hole = editor-only; stencil/apron/2.5D routes captured on the follow-up). Release: `ProjectVersion`→0.0.7-alpha, **`OldAndre` added to `MapsToCook`** (was selectable but uncooked → would fail to load), Win64 Development cook + 7 staged `courses/` dirs. Suite 106/106.
