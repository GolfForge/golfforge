...
- **Latest:** **Range on-turf numbers + criss-cross mow stripes (GOL-28/GOL-168)** (2026-06-06, Windows). `place_range_props.py` now also drops big near-flat **centerline distance numbers** (100/150/200/250 yd, unlit `TextRenderActor`s, collision-less so the ball flies over) alongside the edge stakes — closes **GOL-28**. `build_range_material.py` (a drifted older fork of `build_course_material.py`) was **re-synced** to the GOL-163 engine (MIC + live-tunable `_Tint`/`_Rough` params + `_stripe_mask`) and given a **criss-cross** stripe extension (two perpendicular bands multiplied); range `Fairway` mows criss-cross, tunable on new **`MIC_PracticeRange`**. Same `_stripe_mask` mirrored into the course script (config unchanged) to keep the forks diffable. Editor-Python + assets only, no C++; ran clean in PIE. Before this: golf-ball `M_GolfBall` + CC0 strike SFX + ball-ready badge (GOL-186). Follow-ups: club/face-impact **GOL-187**; remaining connectors **GOL-181** (P4).
