...
- **Latest:** **Golf-ball look + strike SFX + ball-ready UI (GOL-186)** (2026-06-06, Windows). Range-polish batch: the in-flight ball got a real **`M_GolfBall`** material (off-white semi-gloss + a stdlib-generated tiling **dimple normal**, the water-normal pipeline) on the engine sphere — no Fab dependency; a **CC0 ball-strike SFX** (BigSoundBank s0455 trimmed to ~0.55 s via stdlib `aifc`/`audioop`/`wave`) plays one-shot in `OnShotOutcome` for every shot; a **"TAKE YOUR SHOT" ball-ready badge** (top-right, mirrors the Menu chip) driven by a new `OnReadyChanged` seam off the connector's `LaunchMonitorIsReady` heartbeat; and the bottom control bar flattened to a slick full-width strip. Scripts: `build_golfball_assets.py`, `import_ball_strike_sfx.py`. Clean UE 5.7 build; tests green; verified in PIE. Before this: GSPro per-connector profiles + **squaregolf**/**springbok** validated (2 of 6, GOL-181). Follow-ups: club/face-impact **GOL-187**; remaining connectors **GOL-181** (P4).
