...
- **Latest:** Practice Range R3 DONE (2026-05-23, Windows) — perimeter trees framing the range via the existing PCG path. The range got its **own** graph `/Game/PCG/PCG_TreeScatter_Range` (separate from BethPage's to dodge the shared-density footgun), driven by new `scatter_range_trees.py` (persistent via `GenerateOnLoad`). `build_range_splatmap.py`'s layout was reworked from a full 504 m perimeter ring into a tight 400×70-yd open lane framed by a 22-yd tree wall (trees ±35 yd off center; fairway narrowed to 50 yd). **Perf:** 4K fullscreen had dropped to ~45 FPS — GPU/geometry-bound; the **3D fairway grass was ~half the cost and was removed** from the range (grass-output node deleted from `M_PracticeRange`, range-only; `build_range_material.py` defaults `BUILD_GRASS=False`), tree density dialed to **0.10 ppsm (~1,966 trees)** → ~50 FPS / 16 ms GPU. Added an always-on `stat fps` + resolution readout to `AGolfRangeHUD`. Built on R2 (UMG panel + golf-tee cursor) and R1 (tee-fixed arrow aim).
- **Active focus:** practice-range polish, **R4 next** — environment selectors (time-of-day + weather) driven from the R2 panel: add a VolumetricCloud + a small runtime env controller applying Time/Sky presets. See `docs/backlog.md` for the full breakdown.
