...
- **Latest:** **GOL-144 — In-round HUD: round panel + hole-map card (glass)** (2026-06-03, Windows). Eighth child of the GOL-137 UI epic; first of the in-round HUD redesign (top regions). New `UI/RoundHud.{h,cpp}` (full-screen `SelfHitTestInvisible`): glass **round panel** (top-left — avatar/name, `±N THRU n · HCP h`, Menu, HOLE/PAR/YDS/SHOT/TO-PIN row, conditions strip) + **hole-map card** (top-right — placeholder flyover + pin tag + `Hole n · Par n · yds`), replacing the old canvas round text. `GolfRangeHUD` drives it per-Tick from `URoundSubsystem::GetState()` + `GolfDisplay` name/handicap + the env director's **sky/time (real; find-only, no spawn)**; wind/temp are seams ("—"). In a round → show RoundHud + hide the legacy panel; on the range → legacy panel (now a **light glass reskin**, structure/functions intact). Menu → `ReturnToMainMenu`. Commit `1bfe7a9` (full-build + PIE verified). **GOL-137: 8/12 children done (138/139/150/140/141/142/143/144) — history in `docs/worklog.md`.**
- **Active focus:** (1) **GOL-137 UI elevation** (High) — next: **GOL-145** (in-round **bottom** telemetry + control bar + §6 game-vs-LM gating — the ticket that makes the in-round screen match `06`), then **GOL-146** (swing meter glass), **GOL-147** (leave/quit modal — wires the round panel's Menu guard), **GOL-148** (scorecard reskin + epic closeout). Also queued: **GOL-151** (Lucide icon font — retires the placeholder condition/avatar/`→` icons), **GOL-152** (rename Golfsim→GolfForge, cross-machine), **GOL-153** (stepper pill repaint), **GOL-154** (live weather → conditions wind/temp). (2) **v0.0.3-alpha release** — Mac cook (`machine/mac`) then tag + GitHub Release. (3) **GOL-49** — `engine/scripts/package.sh` bundling BuildCookRun + the post-cook `courses/` copy.
