...
- **Latest:** **GOL-162 → 3D grass deferred to GOL-170; Grass Detail toggle backend kept (dormant)** (2026-06-04, Windows). Attempted 3D fairway grass; found it was hand-detached from `M_GolfsimCourse` for dev perf, and UE 5.7's grass-map system (`grass.GrassMap.UseRuntimeGeneration=0`) means grass needs baked maps or runtime-gen enabled to spawn at all — so it never rendered. Per the user, **3D grass is deferred to its own epic GOL-170** (Nanite / runtime-gen vs baked / density-perf investigation) since well-textured surfaces are the bigger beta facelift. Shipped + kept: the **Grass Detail toggle backend** (commit `fcfcf2d`: `FGolfDisplaySettings.GrassDetailLevel` → `grass.densityScale`/`CullDistanceScale`, `golfsim.SetGrassDetail`, GConfig persist, `#if PLATFORM_MAC` default, `FGolfsimModule` boot apply, clamp test) — its Settings UI row is **hidden** until grass renders. Material reverted to its proven grass-free state. Findings → GOL-170 + `docs/ue5-cookbook.md`.
- **Active focus (alpha-3, GOL-160 vibe pass):** next is **GOL-163 surface material upgrade** — distinct, well-textured fairway/rough/green/tee/bunker with their own materials + mowing/region patterns, in `build_course_material.py` (`MaterialEditingLibrary` can't introspect, so the canonical path is re-running that backup→nuke→recreate script). This is the user's priority beta facelift. Then **GOL-164** water, **GOL-165** flag/tree wind, **GOL-166** ambient SFX, **GOL-167** tree species, **GOL-168** props. Siblings: **GOL-93** ball physics, **GOL-94** course authoring (bunker/mowing geometry). Deferred: **GOL-170** 3D grass, **GOL-169** night, **GOL-27** range R4. Other: GOL-152 rename, GOL-153/154, GOL-158/159 (beta).
