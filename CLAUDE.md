...
- **Latest:** **GOL-143 — Round Setup wizard: Players step + Tee Off** (2026-06-03, Windows). Seventh child of the GOL-137 UI epic — **the 3-step Round Setup wizard is now end-to-end** (Course → Format → Players → Tee Off). Players step: count segmented (1 live; 2–4 disabled), one player row (avatar w/ live initials + styled name field + tee swatches + handicap stepper), sticky round-summary card from `FRoundConfig`. `Round/RoundConfig.h` gained `FRoundPlayer`+`PlayerCount`+`Players[]`; `GolfDisplaySettings` gained handicap persistence (`[GolfForge.Round]/Handicap`, 0–54). Tee Off persists `Players[0]` name+handicap (scorecard + menu chip already read it); tee box + multiplayer + net-handicap scoring are GOL-69 seams. Test `Golfsim.Settings.HandicapRoundTrip`. Commit `6a3352c` (full-build + tests + PIE verified). **GOL-137: 7/12 children done (138/139/150/140/141/142/143); the wizard arc is complete — history in `docs/worklog.md`.**
- **Active focus:** (1) **GOL-137 UI elevation** (High) — next: **in-round HUD (GOL-144 round panel/hole map/conditions + GOL-145 telemetry/control bar + §6 LM gating)**, then **GOL-147** (leave/quit modal), **GOL-146** (swing meter reskin), **GOL-148** (scorecard reskin + epic closeout). Also queued: **GOL-151** (Lucide icon font — retires the `→`/`X`/placeholder-icon stopgaps across the menu + wizard), **GOL-152** (project/exe rename Golfsim→GolfForge — cross-machine), **GOL-153** (stepper pill repaint). (2) **v0.0.3-alpha release** — Mac cook (`machine/mac`) then tag + GitHub Release. (3) **GOL-49** — `engine/scripts/package.sh` bundling BuildCookRun + the post-cook `courses/` copy.
