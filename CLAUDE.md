...
- **Active focus:** **EventBus (GOL-7) + manual-shot dialog (GOL-8) shipped; LM framework + OpenFlight (GOL-11) landed** (code + 17/17 tests; live transport blocked on a UE `lws` bug — follow-up filed). Tracks from here:
  - **Unblock GOL-11 live (High, Windows):** resolve the UE-5.7 libwebsockets `WSAENOTSOCK` (errno 10038) connect failure — a `.NET ClientWebSocket` connects to the same OpenFlight URL fine, so the fix is UE-side (investigate lws/Winsock or a 3rd-party Socket.IO/WS plugin). See the follow-up ticket. Until then the OpenFlight parse→bus→panel chain runs via `golfsim.LMSimulate`; Square Omni (GOL-12) can still build on the framework.
