...
- **Two species (Silver Birch + Baltic Pine), 4 distinct variant meshes each = 8 meshes.** The `_A/_B/_C/_D` Megaplants are NOT LOD tiers but distinct individual trees of different heights (Birch_A 2594 tris/14.8 m vs Birch_D 3400 tris/7.2 m) — all four are used as genuine variety. Driven by a `globals()`-overridable `SPECIES` table (dir/prefix/variants/scale band/lean/seed) so course+range share it and a 3rd species (or the deferred static-Nanite branch) is one table row. Verified: all 8 soft paths resolve (birch `SK_` prefix, pine no-prefix asymmetry handled).
- **No `PCGSkinnedMeshSelectorWeighted` in this 5.7 build** (verified) — so species/variant mixing writes the `Mesh` soft-path attribute per point (one `AddAttribute` per mesh) feeding the attribute-driven `SkinnedMeshSpawner`, exactly the proven single-species pattern fanned out. Per-point selection rolls a seeded uniform [0,1] into a **named** attribute (a Density-Noise node aimed at an attribute, not the Density channel) and partitions it with a chain of attribute-vs-constant `AttributeFilter`s using both inside/outside pins. (First cut routed via the Density channel + `DensityFilter` bands and **double-counted** — a `keep_zero` band swept a duplicate pile, ballooning 30k→77k with one variant at 4x; the named-attribute split fixed it.)
- **Parallel per-species branches** so each species gets its own NON-uniform scale band (pines tall+narrow `z 1.05-1.45 / xy 0.80-1.05`, birches rounder) + small random pitch/roll lean — a single shared spawner could only apply one scale. Full random yaw kept.
- **Graded density off paint:** keep where `Trees weight > ZoneRoll`, so keep-probability == paint strength. On the current (effectively binary) Trees paint this is a near-no-op (perimeter-dense + fairway-zero via the hard `Trees>0.3` gate, same as before); it grades rough automatically if/when the Trees layer is painted with graded weights.
- **Verified on the course:** 27-node graph, 8 ISKM components, even variant split (birch ~4.1k each, pine ~3.4k each), **30,457 total** (within the 25k-60k gate band, ~= the old 29k single-species), species mix 55/45 = the `SPECIES_SPLIT` knob; instance transforms confirm independent height/width + lean. Course volume stays transient (measure-only); range regenerates on load (operator saves `PracticeRange.umap`). FPS/GPU gate is operator-read in PIE.
- **Deferred, GOL-167 rolls them up:** static-Nanite tree variant (GOL-31, asset to be sourced) and tree sway (GOL-173). Inspected `Birch_WindSettings_01` = a `PVWindSettings` *parameter* asset, not a WPO enabler — does not bypass the GOL-165 "skinned-spawner instances ignore the foliage WPO" blocker, so sway stays deferred.
...
- **Every hole flagged on the course (`8a5b6e7`).** Reworked `URoundPinSubsystem` from one-moving-pin to **one pin per scheduled hole**: `round.start` spawns a full pin (disc + pole + fluttering flag) at every green (ground-snapped per-hole, since `Schedule[*].PinWorldLoc` is Z=0 until snapped), `hole.start` lights the active hole's gimme ring + collapses the prior one's, `round.complete`/`Deinitialize` destroy all. Keyed by hole `Ref` in a `TMap`; `GetPin()` returns the active pin; find-or-spawn fallback if the schedule's unreadable. Build green, `Golfsim.Round`+`RoundEvents` automation pass.
...
- **Verified:** build green (22.6 s), `Golfsim.Settings` automation passes, grass cvar names + Nanite mesh state confirmed in-editor. Files: `GolfDisplaySettings.{h,cpp}`, `Golfsim.cpp`, `GolfsimConsole.cpp`, `SettingsMenu.{h,cpp}`, `Tests/SettingsTests.cpp`.
- **Outcome — 3D grass DEFERRED to GOL-170.** PIE showed no blades: the grass had been hand-detached from `M_GolfsimCourse` for dev perf, and UE 5.7's grass-map system (`grass.GrassMap.UseRuntimeGeneration=0`) requires baked grass maps or runtime-gen enabled to spawn — neither was set, so 0 instances. Re-running `build_course_material.py` re-wired it cleanly (single grass node) but it still needed runtime-gen/bake, and full density had killed dev perf before. Per the user, 3D grass is its own investigation now (**GOL-170**: Nanite vs traditional, runtime-gen vs baked, density/cull, Mac perf) — well-textured surfaces (GOL-163) are the bigger beta facelift. **Kept** the toggle backend (`fcfcf2d`, dormant, Settings UI hidden); **reverted** the material to its grass-free shipping state. Doc corrections committed in a follow-up.
...
- **Files:** `GolfEnvironment.{h,cpp}` (rename + new features), `GolfsimConsole.cpp` (SetTimeOfDay + help), `GolfRangeHUD.cpp` (type rename), `GolfForgeDemoBlack.umap` (placed atmosphere), `docs/ue5-cookbook.md` (recipe), `docs/worklog.md`, `CLAUDE.md`. Committed across `6937a2f` (generalization) + `7199305` (umap + Night-deferral + docs) + `35e7249` (refreshed course README screenshot, PNG->JPEG q85). **GOL-161 DONE.** The range R4 deep-tune (GOL-27) was consciously deferred this pass — Night went to GOL-169, the rest of the matrix reads acceptably.
